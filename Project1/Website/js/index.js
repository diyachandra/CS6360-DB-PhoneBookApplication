...
$(function() {
  ...
  $("#saveButton").on('click', function() {
    ...
    for (var i = 0; i < currentPhoneLength; ++i) {
      ...
      phoneData += '&' + 'phoneid' + i + '=' + $('#phone' + i).attr('p_id') + '&' +
        `con_id${i}=` + currentGlobalContactid + '&';
    }
    ...
    for (var i = 0; i < currentDateLength; ++i) {
      ...
      dateData += '&' + 'dateid' + i + '=' + $('#date' + i).attr('d_id') + '&' +
        `con_id${i}=` + currentGlobalContactid + '&';
    }
    ...
    for (var i = 0; i < currentAddressLength; ++i) {
      ...
      addressData += '&' + 'addressid' + i + '=' + $('#address' + i).attr('a_id') + '&' +
        `con_id${i}=` + currentGlobalContactid + '&';
    }
  ...
  $.ajax({
    ...
    success: function(data) {
      $.each(data, function(key, value) {
        var record = value["Fname"] + " " + value["Mname"] + " " + value["Lname"];
        var listItem = "<li id=\"" + value["Contact_id"] + "\" class=\"list-group-item list-group-item-action\">" + record + "</li>";
...
function openCreateForm() {
  ...
  var phoneRow = "<div class='form-row' id='phoneRow" + i + "'>" +
    "<div class='form-group col-md-6' id='updatePhoneType" + i + "'>" +
    "<label for='inputPhoneType'>Phone Type</label>" +
    "<select id='inputPhoneType" + i + "' class='form-control' name='PhoneType" + i + "'>" +
    ...
    "</select>" +
    "</div>" +
    "<div class='form-group col-md-6' id='updatePhoneNum" + i + "'>" +
    "<label for='inputPhoneNumber'>Phone Number</label>" +
    "<input name='PhoneNum" + i + "' class='form-control' id='inputPhoneNumber" + i + "'>" +
    "</div>" +
    "</div>";
  $("#createrestSectionDiv").append(phoneRow);

  var addressSec = '<div id="addressSectionDiv' + i + '">' +
    '<div class="form-group" id="updateAddressRow1' + i + '">' +
    '<label for="inputAddress">Address</label>' +
    '<input name="Address' + i + '" class="form-control" id="inputAddress' + i + '" placeholder="1234 Main St">' +
    '</div>' +
    '<div class="form-row" id="updateAddressRow2' + i + '">' +
    '<div class="form-group col-md-3" id="updateAddressType' + '">' +
    '<label for="inputAddressType">Address Type</label>' +
    '<select id="inputAddressType' + i + '" class="form-control" name="AddressType' + i + "\">" +
    '<option selected hidden>Choose...</option>' +
    '<option value="Home">Home</option>' +
    '<option value="Work">Work</option>' +
    '<option value="Other">Other</option>' +
    ...
    '<div class="form-group col-md-3" id="updateCity' + i + '">' +
    '<label for="inputCity">City</label>' +
    '<input name="City' + i + '" class="form-control" id="inputCity' + i + '">' +
    '</div>' +

    '<div class="form-group col-md-3" id="updateState' + i + '">' +
    '<label for="inputState">State</label>' +
    '<select id="inputState' + i + '" class="form-control" name ="State' + i + '">' +
    '<option selected hidden>Choose...</option>' +
    ...
    '                            </select>\n' +
    '                          </div>\n' +
    '<div class="form-group col-md-3" id="updateZip' + i + '">' +
    '<label for="inputZip">Zip</label>' +
    '<input name="Zip' + i + '\" class="form-control" id="inputZip' + i + '">' +
    '</div>\n' +
    '\n' +
    '                        </div>\n' +
    '                      </div>\n';
  $("#createrestSectionDiv").append(addressSec);
  var dateRow = '<div class="form-row" id="dateSectionDiv' + i + '">' +
    '<div class="form-group col-md-6" id="updateDateTypeRow' + i + '">' +
    '<label for="inputDateType">Date Type</label>' +
    '<select id="inputDateType' + i + '" class="form-control" name="DateType' + i + '">' +
    '<option selected hidden>Choose</option>' +
    ...
    '</select>\n' +
    '</div>\n' +
    '<div class="form-group col-md-6" id="updateDateRow' + i + '">' +
    '<label for="inputDate">Date</label>' +
    '<input name="Date' + i + '" class="form-control" id="inputDate' + i + '">' +
    '</div>\n' +
    '</div>\n';
...
function loadUpdateForm(formData) {
  ...
  var addphoneButton = '<button type="button" name="delete" id="addphoneButton" class="btn btn-primary">Add Phone</button>';
  var delphoneButton = '<button type="button" name="delete" id="delphoneButton" class="btn btn-danger">Delete Phone</button>';
  ...
  if (phonelen == 0) {
    var phoneRow = "<div class='form-row' id='phoneRow'>" +
  ...
  for (i = 0; i < phonelen; i++) {
    var phoneRow = "<div class='form-row' id='phoneRow" + i + "'>" +
      ...
      "</div>";
    $("#restSectionDiv").append(phoneRow);
    $(`#inputPhoneType${i} option[value='${formData["phones"][i][0]}']`).prop("selected", true);
    $("#inputPhoneNumber" + i).val(formData["phones"][i][2]);

  }
  var addaddressButton = '<button type="button" name="delete" id="addaddressButton" class="btn btn-primary">Add Address</button>';
  var deladdressButton = '<button type="button" name="delete" id="deladdressButton" class="btn btn-danger">Delete Address</button>';

  $("#restSectionDiv").append(addaddressButton);
  $("#restSectionDiv").append(deladdressButton);
  var addrlen = formData['addresses'].length;
  if (addrlen == 0) {
    var addressSec = '<div id="addressSectionDiv">' +
  ...
  for (i = 0; i < addrlen; i++) {
    ...
    $(`#inputAddressType${i} option[value='${formData["addresses"][i][0]}']`).prop("selected", true);
    $("#inputCity" + i).val(formData['addresses'][i][2]);
    $(`#inputState${i} option[value='${formData["addresses"][i][3]}']`).prop("selected", true);
    $("#inputZip" + i).val(formData['addresses'][i][4]);

  }
  var adddateButton = '<button type="button" name="delete" id="adddateButton" class="btn btn-primary">Add Date</button>';
  var deldateButton = '<button type="button" name="delete" id="deldateButton" class="btn btn-danger">Delete Date</button>';

  $("#restSectionDiv").append(adddateButton);
  $("#restSectionDiv").append(deldateButton);
  var datelen = formData["dates"].length;
  if (datelen == 0) {
    var dateRow = '<div class="form-row" id="dateSectionDiv">' +
  ...
  for (i = 0; i < datelen; i++) {
    var dateRow = '<div class="form-row" id="dateSectionDiv' + i + '">' +
      ...
      '</div>\n';
    $("#restSectionDiv").append(dateRow);
    $(`#inputDateType${i} option[value='${formData["dates"][i][0]}']`).prop("selected", true);
  ...
  $('#addaddressButton').on('click', function() {
    ...
    var addressRow = '<div id="addressSectionDiv' + i + '">' +
      '<div class="form-group" id="updateAddressRow1' + i + '">' +
      '<label for="inputAddress">Address</label>' +
      '<input name="Address' + i + '" class="form-control" id="inputAddress' + i + '" placeholder="1234 Main St">' +
      '</div>' +
...
function loadPhone(details) {
  ...
  if (len > 0) {
    for (i = 0; i < len; i++) {
      var item = "<tr p_id='" + details['phones'][i][3] + "' id='phone" + i + "'><td width=\"33%\"> Phone Type: " + details["phones"][i][0] +
        "</td><td width=\"33%\"> Area Code: " + details["phones"][i][1] +
        "</td><td width=\"33%\"> Phone Number: " + details["phones"][i][2] + "</td></tr>";
  ...
  if (len > 0) {
    for (i = 0; i < len; i++) {
      ...
      if (details["addresses"][i][0] != "") {
        item += "<tr a_id='" + details["addresses"][i][5] + "' id='address" + i + "' ><td width=\"33%\"> Address Type: " + details["addresses"][i][0] + "</td>";
      }
      if (details["addresses"][i][1] != "") {
        item += "<td width=\"33%\"> Address: " + details["addresses"][i][1] + "</tr>";
      }
      if (details["addresses"][i][2] != "") {
        item += "<tr></td><td width=\"33%\"> City: " + details["addresses"][i][2] + "</td>";
      }
      if (details["addresses"][i][3] != "") {
        item += "<td width=\"33%\"> State: " + details["addresses"][i][3] + "</td>";
      }
      if (details["addresses"][i][4] != "") {
        item += "<td width=\"33%\"> Zip: " + details["addresses"][i][4] + "</td></tr>";
      }
  ...
  if (len > 0) {
    for (i = 0; i < len; i++) {
      ...
      if (details["dates"][i][0] != "") {
        item += `<tr d_id=${details["dates"][i][2]} id='date${i}'><td width='33%'> Date Type: ` + details["dates"][i][0] + "</td>";
      }
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $.each(data, function(key, value) {
        ...
        var listItem = "<li id=\"" + value[0] + "\" class=\"list-group-item list-group-item-action\">" + record + "</li>";
        $("#contactsList").append(listItem);
      });
      $('.list-group-item').on('click', function() {
        // alert($(this).text());
